function Slide(selector, opts) {
  //参数类型：selector-目标元素选择器，opt-包含一个图片信息数组的对象
...
Slide.prototype.createNode = function() {
  //创建轮播器的内容
...
Slide.prototype.move = function(index) {
  ...
  if (_this.cur >= _this.len) {
    //当图片到最后一张的时候，将oUl的left值设为0重新开始
    images.style.left = 0;
    _this.cur = 1;
  } else if (_this.cur == -1) {
    //当图片到第一张的时候，将oUl的left值设为最后一张重新开始
...
Slide.prototype.paused = function() {
  //暂停函数
...
function getStyle(obj, stylename) {
  ...
  return obj.currentStyle
    ...
    : getComputedStyle(obj)[stylename];
}
...
function doMoveCall(obj, attr, dir, target, endFn) {
  //元素运动回调传参
